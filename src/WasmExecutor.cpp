...
void add_extern_callbacks(const Local<Context> &context,
    ...
    for (const auto &it : jit_externs) {
        ...
        imports_dict->Set(context, String::NewFromUtf8(isolate, name.c_str()), f).Check();
    }
}
...
WasmModuleContents::WasmModuleContents(
    ...
    const auto add_callback = [&](const char *name, FunctionCallback f) {
        ...
        while (*name == ':') name++;
        imports_dict->Set(context, String::NewFromUtf8(isolate, name),
                      FunctionTemplate::New(isolate, f)->GetFunction(context).ToLocalChecked()).Check();
    };
...
#undef ADD_POSIX_MATH2
    ...
    imports->Set(context, String::NewFromUtf8(isolate, "env"), imports_dict).Check();
